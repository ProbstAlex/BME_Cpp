...
void compute_2_1_1_a(const int* values, int nbrOfValues, int& sum, int& min, int& max)
{
    ...
    {
        sum += values[i]; // compute sum
        (values[i] < min) ? min = values[i] : min; // compute min
        (values[i] > max) ? max = values[i] : max; // compute max
    }
}

void compute_2_1_1_b(const int* values, int nbrOfValues, int& sum, int& min, int& max)
{
    //compute by pointer arithmetic
    for(int i = 0; i < nbrOfValues; ++i)
    {
        sum += *values + i;
        (*(values + i) < min) ? min = *(values + i) : min; // compute min
...
{
    //compute by receiving smart pointer
    for(int i = 0; i < nbrOfValues; ++i)
    {
        sum += values[i];
        (values[i] < min) ? min = values[i] : min; // compute min
...
void compute_2_1_3(const std::vector<int>& values, int& sum, int& min, int& max)
{
    ...
    {
        sum += values;
        (values < min) ? min = values : min; // compute min
        (values > max) ? max = values : max; // compute max
    }
}
...
/* todo:  Answer the questions under 2.2 -> as shorter as better :)
    ...
    The pointer variable has the value of the adress it is pointing to.
    To assign a value to the memory address the pointer is pointing to, one has to
    use *pointToAnInt = 9; in line 6. This is called dereferencing.

    Why and where does the program crash? ->

    In line 5, the syntax does not initialize a pointer. The comiler will not know what to do
    with "new int" if it is not assigned to a pointer. Therefore we have to correct the line to:
    ...
    {
        int * temp = new int [ length ]; // Correction: allocation needs to be a pointer. Scope not accesible in function.
        return temp ;
    }

    Hoopla! ->

    int array [5] { 0, 1, 2, 3 }; // an array of 5 needs 5 values
    for ( int count = 0; count <= 5; ++ count ) // counts up to 6 iterations (0,1,2,3,4,5)
    {
...
int main ()
{
    ...
    int* arrValues = new int[nbrOfValues]; // initial dynamic allocation
